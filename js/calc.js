...
google.maps.event.addDomListener(window, 'load', function (){
  ...
  google.maps.event.addListener(autocomplete, 'place_changed', recalc);
  google.maps.event.addListener(autocomplete2, 'place_changed', recalc);
});
var service = new google.maps.DistanceMatrixService();
$("#from, #where, #volume, #kind").on('change', recalc);
// =============================================================================
...
function callback(response, status) {
  ...
  var volume = parseInt(jQuery('#volume').val());
  var distance = 0;
  var total_sum = 0;
  //
  var errs = [];
  ...
  if (errs.length == 0) {
    if (errsImportant.length == 0) {
      ...
      //
        var qty = jQuery('#edit-qty').val();
      jQuery('#price').html(number_format(parseInt(total_sum), 2, '.', ' ') + ' <i class="fa fa-rub" aria-hidden="true"></i>');
        jQuery('#plus_price_val').html(number_format(parseInt(total_sum)/qty, 2, '.', ' '));
